...
// ReSharper disable once InconsistentNaming
    ...
    public class INIFile : IINIFile
    {
        ...
        private static readonly Dictionary<ParseStrategies, ILineParser>
            Parsers = new()
            {
                [ParseStrategies.BestEffort] = new BestEffortLineParser(), [ParseStrategies.Strict] = new StrictLineParser()
            };
        ...
        private readonly HashSet<string> _wasEscaped = new();
        ...
        public IDictionary<string, string> this[string index]
        {
            get
            {
                ...
                {
                    if (_sectionWrappers.TryGetValue(index, out var wrapper)) return wrapper;
            ...
            params string[] comments)
        {
            ...
            if (comments.Any()) StoreCommentsForSection(section, comments, lineParser.CommentDelimiter);
        }
        ...
        {
            if (existingName is null || newName is null) return;
        ...
        )
        {
            ...
            if (removeEmptySection && !sectionData.Any()) Data.Remove(section);
        ...
        {
            if (!HasSetting(section, key)) return defaultValue;
        ...
        {
            ParseStrategy = parseStrategy;
            if (!string.IsNullOrWhiteSpace(path)) Load(path, parseStrategy);
        }
        ...
        private void Parse(IEnumerable<string> lines)
        {
            ...
            foreach (var line in lines.Where(l => l != null))
            {
                ...
                if (!string.IsNullOrWhiteSpace(parsedLine.Comment)) recentComments.Add(parsedLine.Comment);
        ...
        {
            if (!parsedLine.ContainedEscapedEntities) return;
        ...
        )
        {
            ...
            foreach (var section in sections)
            {
                ...
                if (section.Length > 0) lines.Add($"[{section}]");
                ...
                if (addSeparator) lines.Add(sep);
            }

            if (addSeparator && lines.Any()) lines.RemoveAt(lines.Count - 1);

            if (AppendTrailingNewLine) lines.Add("");
        ...
        {
            if (string.IsNullOrWhiteSpace(sectionSeparator)) return sectionSeparator;
